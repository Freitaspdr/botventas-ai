...
export async function getDashboardStats(period: '7d' | 'today' | '30d' = 'today') {
  ...
  const [convRes, hotRes, leadsRes, transferRes, citasRes, convPrev, hotPrev] = await Promise.all([
    ...
    supabase.from('leads').select('id, estado', { count: 'exact' }).eq('empresa_id', empresaId),
    supabase.from('conversaciones').select('id', { count: 'exact', head: true })
  ...
  return {
    ...
    conversion: { value: total > 0 ? Math.round((cerrados / total) * 100) : 0 },
    tasaBot: { value: convVal > 0 ? Math.round(((convVal - transfer) / convVal) * 100) : 100 },
  };
}
...
export async function getDashboardRevenue() {
  ...
  return {
    ...
    cambio,
    roi,
    plan: String(r.plan_nombre ?? 'starter'),
    convUsadas,
...
export async function getConversacionDetalle(id: string) {
  ...
  return data as {
    ...
    contacto: { id: string; telefono: string; nombre: string | null; estado: string; origen: string; notas: string | null; etiquetas: string[] | null; ultima_interaccion_en: string | null } | null;
    mensajes: { rol: 'user' | 'assistant' | 'human'; contenido: string; enviado_en: string }[];
...
export async function getContactos(params: { q?: string; estado?: string } = {}) {
  const empresaId = await getEmpresaId();
  if (!empresaId) return [];

  const { data } = await supabase.rpc('get_contactos', {
    p_empresa_id: empresaId,
    p_q: params.q ?? null,
    p_estado: params.estado ?? null,
  });
...
export async function getEmpresa(): Promise<EmpresaData | null> {
  ...
  return {
    ...(safeData as unknown as Omit<EmpresaData, 'has_evolution_api_key'>),
